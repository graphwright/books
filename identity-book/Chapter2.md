...
The loader handles this with a fixpoint loop -- a loop that reruns the same step until another pass would change nothing, which is the point at which it has reached a fixpoint. On the first pass, all first-order predicates are hydrated immediately. Higher-order predicates (those whose `subject` or `object_` type annotation is `BaseStatement` or a subclass) are deferred. The deferred list is then retried in a loop until it stops shrinking:
